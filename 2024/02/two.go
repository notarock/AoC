...
func main() {
	...
	for i := 0; i < len(input); i++ {
		...
		if !safep {
			fmt.Println("Initial slice is not safe:", input[i])
			for j := 0; j < len(input[i]); j++ {
				copied := append([]int(nil), input[i]...)

				a := removeElement(copied, j)
				fmt.Println("Testing after removal:", a)

				safep = IsSafe(a)
				if safep {
					fmt.Println("Found safe slice:", a)
					break
				}
			}
		}
...
func removeElement(baseSlice []int, index int) []int {
	return append(baseSlice[:index], baseSlice[index+1:]...)
}
